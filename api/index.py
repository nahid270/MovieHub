...
def dashboard():
    ...
    ad_count = ads_collection.count_documents({}) if ads_collection is not None else 0
...
def manage_ads():
    if request.method == 'POST' and ads_collection is not None:
        ...
        flash('Database connection is not available.', 'error')
    all_ads = list(ads_collection.find()) if ads_collection is not None else []
    return render_admin_page(ADMIN_ADS_CONTENT, title="Manage Ads", ads=all_ads)

@admin_bp.route('/ads/delete/<ad_id>', methods=['POST'])
@login_required
def delete_ad(ad_id):
    if ads_collection is not None: ads_collection.delete_one({"_id": ObjectId(ad_id)}); flash('Ad deleted.', 'success')
...
def home():
    ...
    try:
        ...
        print(f"Successfully fetched {len(movies)} movies from DB.")
    except Exception as e:
        print(f"ERROR: Failed to fetch movies from DB. Reason: {e}")
        return render_template_string(PAGE_TEMPLATE, title="Application Error", error_message=f"Could not fetch movies from the database. It might be a timeout issue. Please try again. Error: {e}")
