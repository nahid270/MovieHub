...
def search_tmdb(query, type): return tmdb_get(f"/search/{type}", params={"query": query, "include_adult": "false"}).get("results", [])
...
def manage_content():
    ...
    if content_collection is not None:
        ...
        for item in site_content:
            item['categories_names'] = [all_categories[cat_id]['name'] for cat_id in item.get('categories', []) if cat_id in all_categories]
    ...
    try:
        if content_collection.find_one({"tmdb_id": tmdb_id, "type": type}):
            ...
            if details:
                ...
                # Auto-create and assign categories from TMDB genres
                category_ids = []
                for genre_name in new_content.get('categories_names', []):
                    categories_collection.update_one({"name": genre_name}, {"$setOnInsert": {"name": genre_name}}, upsert=True)
                    category = categories_collection.find_one({"name": genre_name})
                    if category: category_ids.append(category['_id'])
                
                new_content['categories'] = category_ids
                content_collection.insert_one(new_content)
                flash(f"'{new_content['title']}' added and auto-categorized!", 'success')
            else:
...
def edit_content(content_id):
    # This function is now only for adding/editing links.
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            flash('Content links updated successfully.', 'success')
            return redirect(url_for('admin.manage_content'))
        
        return render_admin_page(ADMIN_EDIT_CONTENT_TEMPLATE, title="Edit Content", content=item)
    except Exception as e:
...
def home():
    ...
    try:
        if categories_collection is not None and content_collection is not None:
            ...
            all_categories = list(categories_collection.find().sort('name', 1)) # Sort categories alphabetically
...
def content_detail(content_id):
    ...
    try:
        ...
        # Fetch category names for the detail page
        all_categories = {cat['_id']: cat for cat in categories_collection.find()}
        item['categories_names'] = [all_categories[cat_id]['name'] for cat_id in item.get('categories', []) if cat_id in all_categories]
        
        return render_template_string(CONTENT_DETAIL_TEMPLATE, content=item)
    except Exception as e:
