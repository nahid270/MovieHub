...
def map_content_from_tmdb(m, type):
    ...
    return {
        ...
        "categories_names": genres, "categories": []
    }
...
def manage_content():
    ...
    return render_admin_page(ADMIN_CONTENT_TEMPLATE, title="Manage Content", site_content=site_content, tmdb_results=tmdb_results)
    ...
    try:
        if content_collection.find_one({"tmdb_id": tmdb_id, "type": type}):
            ...
            if details:
                ...
                if genres: # শুধুমাত্র প্রথম genre টি যোগ করা হচ্ছে
...
def edit_content(content_id):
    ...
    try:
        ...
        return render_admin_page(ADMIN_EDIT_CONTENT_TEMPLATE, title="Edit Content", item=item, all_categories=all_categories)
    except Exception as e:
    ...
    try:
        if content_collection is not None:
            ...
            for cat in all_categories:
                ...
                if content_in_cat:
                    # Avoid duplicating the "Movies" and "TV Series" shelves if they exist as genres
                    if cat['name'] not in ["Latest Movies", "Latest TV Series"]:
...
def content_detail(content_id):
    ...
    try:
        ...
        all_categories = {cat['_id']: cat for cat in categories_collection.find()}
        item['categories_names'] = [all_categories.get(cat_id, {}).get('name', '') for cat_id in item.get('categories', [])]
