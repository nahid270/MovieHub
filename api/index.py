...
ADMIN_BASE_TEMPLATE = """<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{{ title or "Admin Panel" }} • Movie Zone</title><script src="https://cdn.tailwindcss.com"></script><meta name="color-scheme" content="dark"></head><body class="bg-gray-900 text-gray-200 antialiased"><div class="flex min-h-screen"><aside class="w-64 bg-gray-950 p-6 border-r border-white/10 flex flex-col"><h1 class="text-2xl font-bold mb-8">🎬 Admin Panel</h1><nav class="flex flex-col gap-2"><a href="{{ url_for('admin.dashboard') }}" class="px-4 py-2 rounded-lg hover:bg-white/10">Dashboard</a><a href="{{ url_for('admin.manage_movies') }}" class="px-4 py-2 rounded-lg hover:bg-white/10">Manage Movies</a><a href="{{ url_for('admin.manage_categories') }}" class="px-4 py-2 rounded-lg hover:bg-white/10">Manage Categories</a></nav><div class="mt-auto"><a href="{{ url_for('home') }}" target="_blank" class="block w-full text-center px-4 py-2 rounded-lg hover:bg-white/10 text-sm mb-2">View Site ↗</a><a href="{{ url_for('admin.logout') }}" class="block w-full text-center px-4 py-2 rounded-lg bg-red-600/50 hover:bg-red-600/80 text-sm">Logout</a></div></aside><main class="flex-1 p-8">{% with messages = get_flashed_messages(with_categories=true) %}{% if messages %}{% for category, message in messages %}<div class="mb-4 p-4 rounded-lg {% if category == 'success' %}bg-green-500/20 text-green-300 border border-green-500/30{% elif category == 'error' %}bg-red-500/20 text-red-300 border border-red-500/30{% else %}bg-blue-500/20 text-blue-300 border border-blue-500/30{% endif %}">{{ message }}</div>{% endfor %}{% endif %}{% endwith %}{{ content | safe }}</main></div></body></html>"""
...
client, db, movies_collection, categories_collection, DB_CONNECTION_ERROR = None, None, None, None, None
if MONGO_URI:
    try:
...
def dashboard():
    ...
    return render_admin_page(ADMIN_DASHBOARD_CONTENT, title="Dashboard", movie_count=movie_count, category_count=category_count)
...
def manage_movies():
    ...
    return render_admin_page(ADMIN_MOVIES_CONTENT, title="Manage Movies", site_movies=site_movies, tmdb_results=tmdb_results, all_categories=all_categories)
    ...
    try:
        if movies_collection.find_one({"tmdb_id": tmdb_id}): flash('Movie already exists.', 'warning')
            ...
            if movie_details:
                ...
                flash('Movie added successfully! Now edit it to add links and categories.', 'success')
            else: flash('Could not fetch movie details.', 'error')
    except Exception as e:
        flash(f"Failed to add movie. Check DB user permissions. Error: {e}", 'error')
    return redirect(url_for('admin.manage_movies'))
...
def edit_movie(movie_id):
    ...
    try:
        ...
        return render_admin_page(ADMIN_EDIT_MOVIE_TEMPLATE, title="Edit Movie", movie=movie, all_categories=all_categories)
    except Exception as e:
...
def manage_categories():
    if request.method == 'POST' and categories_collection is not None:
        ...
        if cat_name:
            if categories_collection.find_one({"name": cat_name}):
                flash("Category with this name already exists.", "warning")
            else:
                categories_collection.insert_one({"name": cat_name})
...
def delete_category(category_id):
    if categories_collection is not None:
        # Also remove this category from all movies that have it
        movies_collection.update_many({}, {"$pull": {"categories": ObjectId(category_id)}})
        categories_collection.delete_one({"_id": ObjectId(category_id)})
        flash("Category and its associations deleted.", "success")
    return redirect(url_for('admin.manage_categories'))
...
def pick_banners(movie_list, need=5):
    ...
    for m in movie_list:
        if m.get("_id"):
            banners.append({"src": m.get("backdrop") or m.get("poster"), "title": m.get("title"), "overview": m.get("overview"), "movie_id": str(m.get("_id"))})
...
def home():
    ...
    banner_movies = []
    try:
        if categories_collection is not None and movies_collection is not None:
            banner_movies = list(movies_collection.find().sort('_id', DESCENDING).limit(5))
            all_categories = list(categories_collection.find())
            for cat in all_categories:
                movies_in_cat = list(movies_collection.find({"categories": cat['_id']}).sort('_id', DESCENDING).limit(10))
                if movies_in_cat:
                    cat['movies'] = movies_in_cat
                    categories_with_movies.append(cat)
    except Exception as e:
        return render_template_string(PAGE_TEMPLATE, error_message=f"Could not fetch data. Error: {e}")
    return render_template_string(PAGE_TEMPLATE, title="Movie Zone", banners=pick_banners(banner_movies), categories=categories_with_movies)
