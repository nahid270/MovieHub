...
def manage_content():
    ...
    if content_collection is not None:
        ...
        for item in site_content:
            item['categories_names'] = [all_categories.get(cat_id, {}).get('name', 'Unknown') for cat_id in item.get('categories', [])]
...
def edit_content(content_id):
    ...
    try:
        ...
        if not item:
            ...
            return redirect(url_for('admin.manage_content'))
        if request.method == 'POST':
            auto_categories = item.get('categories', [])
            manual_categories = [ObjectId(cat_id) for cat_id in request.form.getlist('categories')]
            all_cat_ids = list(set(auto_categories + manual_categories))
            updated_data = {
